...
export function RegretChart() {
  ...
  const yLabel = isRevenue ? 'Cumulative regret ($ lost)' : 'Cumulative regret (clicks lost)';
  ...
  const subtitle = 'Reward lost vs always picking the optimal arm — lower is better';

  if (data.length === 0) {
    return (
      <ChartContainer title="Cumulative Regret" subtitle={subtitle}>
